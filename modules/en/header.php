<div class="preloader"></div>
<!-- Main header-->
<header class="main-header header-style-one">
    <!--Start Header Top-->
    <div class="header-top">
        <div class="auto-container">
            <div class="outer-box clearfix">

                <div class="header-top-left pull-left">
                    <div class="logo">
                        <a style="color:white;" href="/">
                            <img style="width: 8rem;" src="/assets/images/resources/logo2.png" alt="window" title="">
                        </a>
                    </div>
                </div>
                <div class="header-top-right pull-right">
                    <div class="header-contact-info clearfix">
                        <ul class="clearfix">
                            <li>
                                <div class="icon">
                                    <span class="flaticon-telephone thm-clr"></span>
                                </div>
                                <div class="text">
                                    <p>Call Us</p>
                                    <h4><a href="tel:6121552640">612 155 2640</a></h4>
                                </div>
                            </li>
                            <li>
                                <div class="icon">
                                    <span class="flaticon-email thm-clr"></span>
                                </div>
                                <div class="text">
                                    <p>Mail</p>
                                    <h4><a
                                            href="mailto:vgzburquez@windowcoveringsbcs.com">vgzburquez@windowcoveringsbcs.com</a>
                                    </h4>
                                </div>
                            </li>
                            <li>
                                <div class="icon">
                                    <span class="flaticon-pin-1 thm-clr"></span>
                                </div>
                                <div class="text">
                                    <p>Address</p>
                                    <h4>Los Barriles, B.C.S</h4>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!--End header Top-->

    <div class="header">
        <div class="auto-container">
            <div class="outer-box clearfix">
                <!--Start Header Left-->
                <div class="header-left clearfix pull-left">
                    <div class="nav-outer clearfix">
                        <!--Mobile Navigation Toggler-->
                        <div class="mobile-nav-toggler">
                            <div class="inner">
                                <span class="icon-bar"></span>
                                <span class="icon-bar"></span>
                                <span class="icon-bar"></span>
                            </div>
                        </div>

                        <!-- Main Menu -->
                        <nav class="main-menu style1 navbar-expand-md navbar-light">
                            <div class="collapse navbar-collapse show clearfix" id="navbarSupportedContent">
                                <ul class="navigation clearfix">
                                    <li class="<?echo $inicio?>"><a href="/">Home</a></li>
                                    <!-- <li><a href="/">Productos</a></li> -->
                                    <li class="<?echo $nosotros?>"><a href="/about">About Us</a></li>
                                    <li class="<?echo $servicios?>"><a href="/services">Services</a></li>
                                    <li class="<?echo $galeria?>"><a href="/gallery">Gallery</a></li>
                                    <li hidden class="<?echo $blog?>"><a href="/blog">Blog</a></li>
                                    <li class="<?echo $medir?>"><a href="/medir">Measure</a></li>
                                    <li class="<?echo $contacto?>"><a href="/contact">Contact Us</a></li>
                                    <li class="dropdown current"><a href="#">Idioma</a>
                                        <ul>
                                            <li><a href="?lang=en">Inglés</a></li>
                                            <li><a href="?lang=es">Español</a></li>
                                        </ul>
                                    </li>
                                </ul>
                            </div>
                        </nav>
                        <!-- Main Menu End-->

                    </div>

                </div>
                <!--End Header Left-->

                <!--Start Header Right-->
                <div class="header-right pull-right clearfix">
                    <div class="header-social-link">
                        <ul>
                            <li>
                                <a href="https://www.facebook.com/SenzPersianasLPZ/" target="_blank"><i
                                        class="fa fa-facebook" aria-hidden="true"></i></a>
                            </li>
                            <li>
                                <a href="https://instagram.com/windowcoverings_bcs?igshid=MzRlODBiNWFlZA=="
                                    target="_blank"><i class="fa fa-instagram" aria-hidden="true"></i></a>
                            </li>
                            <li >
                                <a href="?lang=es"><img src="/assets/images/mex.png" alt=""></a>
                            </li>
                        </ul>
                    </div>
                    <div class="button">
                        <a class="btn-one style2" href="/contact"><span class="txt">Book Now</span></a>
                    </div>
                </div>
                <!--End Header Right-->

            </div>
        </div>
    </div>
    <!--End header -->
    <!--Sticky Header-->
    <div class="sticky-header">
        <div class="container">
            <div class="clearfix">
                <!--Logo-->
                <div class="logo float-left">
                    <a style="color: white;" href="/" class="img-responsive">
                        <img style="width: 8rem;" src="/assets/images/resources/logo2.png" alt="window" title="">
                    </a>
                </div>
                <!--Right Col-->
                <div class="right-col float-right">
                    <!-- Main Menu -->
                    <nav class="main-menu clearfix">
                        <!--Keep This Empty / Menu will come through Javascript-->
                    </nav>
                </div>
            </div>
        </div>
    </div>
    <!--End Sticky Header-->

    <!-- Mobile Menu  -->
    <div class="mobile-menu">
        <div class="menu-backdrop"></div>
        <div class="close-btn"><span class="icon flaticon-multiply"></span></div>
        <nav class="menu-box">
            <div class="nav-logo">
                <a href="/">
                    <img style="width: 8rem;" src="/assets/images/resources/logo2.png" alt="window" title="">
                </a>
            </div>
            <div class="menu-outer">
                <!--Here Menu Will Come Automatically Via Javascript / Same Menu as in Header-->
            </div>
            <!--Social Links-->
            <div class="social-links">
                <ul class="clearfix">
                    <li><a href="https://www.facebook.com/SenzPersianasLPZ/" target="_blank"><span
                                class="fab fa fa-facebook-square"></span></a></li>
                    <li><a href="https://instagram.com/windowcoverings_bcs?igshid=MzRlODBiNWFlZA=="
                            target="_blank"><span class="fab fa fa-instagram"></span></a></li>
                </ul>
            </div>
        </nav>
    </div>
    <!-- End Mobile Menu -->
</header>